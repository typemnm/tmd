...
- 파일 열기/다른 이름으로 저장 다이얼로그에 경로 인라인 자동완성 추가 (→ 화살표로 확정)
...
- 경로 자동완성 및 파일 열기/저장 다이얼로그의 Enter 제출 경로 모두에서, 존재하지 않는 사용자를 가리키는 `~user` 형태의 경로를 입력하면 앱 전체가 크래시하던 문제 수정
- Ctrl+F 검색 다이얼로그에 경로 자동완성이 실수로 붙어 검색어가 파일명으로 덮어써지던 문제 수정
- Alt+G/Alt+I가 Kitty 키보드 프로토콜을 지원하지 않는 터미널에서 전혀 동작하지 않고 선택 영역을 문자로 덮어쓰던 문제 수정 (바인딩에 `priority=True` 추가)
